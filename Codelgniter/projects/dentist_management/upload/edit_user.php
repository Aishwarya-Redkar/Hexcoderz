...
$result=mysqli_query($conn,"UPDATE upload set title='" . $_POST["title"][$i] . "', file='" . $_POST["file"][$i] . "' WHERE id='" . $_POST["id"][$i] . "'");
}
...
<?php
$rowCount = count($_POST["upload"]);
for($i=0;$i<$rowCount;$i++) {
$result = mysqli_query($conn,"SELECT * FROM upload WHERE id='" . $_POST["upload"][$i] . "'");
...
<td><input type="text" name="file[]" class="txtField" value="<?php echo $row[$i]['file']; ?>"></td>
</tr>
